...
//进程切换
//从就绪进程中选择一个优先级最高的进程，来代替当前进程运行
//RR和优先级调度
asmlinkage void schedule(void)
{
	struct schedule_data * sched_data;
	struct task_struct *prev, *next, *p;//*prev调度之前进程，*next调度之后进程
	...
	if (!current->active_mm) BUG();//进程的active_mm为空，出错
need_resched_back:
	prev = current;//变量初始化
	 ...
	 */
	sched_data = & aligned_data[this_cpu].schedule_data;//保证一次运行一个进程

	spin_lock_irq(&runqueue_lock);
	//释放内核锁，开this_cpu中断，并锁住runqueue队列

	/* move an exhausted RR process to be last.. */
	//RR轮转
	if (unlikely(prev->policy == SCHED_RR))//是否为RR
		if (!prev->counter) {
			prev->counter = NICE_TO_TICKS(prev->nice);
			move_last_runqueue(prev);
		}
	//prev->counter任务的时间配额，逐渐减少，
	//当为0：从当前位置到runqueue的prev（末尾？）
	//NICE_TO_TICKS：优先级换算为时间配额，即恢复开始的时间配额

	switch (prev->state) {
		case TASK_INTERRUPTIBLE://可中断睡眠状态
			if (signal_pending(prev)) {
				prev->state = TASK_RUNNING;
				break;
			}
			//查看当前状态state，
			//TASK_INTERRUPTIBLE+有信号：等待处理，内核将状态设置为TASK_RUNNING
		default:
			del_from_runqueue(prev);
		case TASK_RUNNING:;//就绪状态
	}
	prev->need_resched = 0;
	//如果处在TASK_RUNNING，继续
	//如果没有signal_pending信号等待，从等待队列里撤出
...
repeat_schedule:
	...
	c = -1000;//进程的综合权值
	//调度之前将要调度的进程默认为0号？，权值为-1000
	//0号进程既不睡眠，也不死亡
	list_for_each(tmp, &runqueue_head) {
		p = list_entry(tmp, struct task_struct, run_list);
		if (can_schedule(p, this_cpu)) {
			int weight = goodness(p, this_cpu, prev->active_mm);
			//内遍历可执行队列runqueue的每个进程，用godness计算权值
			if (weight > c)//比较权值
				c = weight, next = p;
		}
	}
	...
	if (unlikely(!c)) {
		...
		for_each_task(p)//对每个进程进行操作
			p->counter = (p->counter >> 1) + NICE_TO_TICKS(p->nice);
		read_unlock(&tasklist_lock);
		spin_lock_irq(&runqueue_lock);
		goto repeat_schedule;
	}//若c=0，说明运行队列中所有权值都为0，重新分配
	...
	spin_unlock_irq(&runqueue_lock);//队列解锁，开中断
	 ...
	 */
	prepare_to_switch();//进程切换
	{
		struct mm_struct *mm = next->mm;//一般进程，切换到next的用户空间
		struct mm_struct *oldmm = prev->active_mm;//内核进程（线程？），借用prev的地址空间
		if (!mm) {
		...
		if (!prev->mm) {//内核进程（线程？）
			prev->active_mm = NULL;//释放地址空间
			mmdrop(oldmm);//mm_struct的共享计数器减一
		}
	}
...
same_process:
	...
	return;//针对smp（smp是多处理机系统），如果调度标志位不为0，重新调度
}
